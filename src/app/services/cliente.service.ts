...
getSubtotal(): number {
  ...
  async insertPedido() {
    ...
    const idMesa = await this.getMesaID(idCliente);
    
    // Eliminar cualquier pedido rechazado anterior antes de insertar el nuevo
    try {
      await this.eliminarPedidoRechazado();
      console.log('🗑️ Pedidos rechazados anteriores eliminados');
    } catch (error) {
      console.log('⚠️ No había pedidos rechazados para eliminar o error:', error);
    }
    
    // Calcular totales
    const subtotal = this.getSubtotal();
    const totalConDescuento = await this.getTotal();
    const descuento = await this.getDescuentoCliente(idCliente, idMesa);
